...
class ContainerTest extends \PHPUnit_Framework_TestCase{
	...
	public function testDefinableContainer(){
		...
		$this->assertEquals('stdClass', get_class($service),
			'ClassName Definition: service [std] is not a "strClass" instance'
		);
		...
		$this->assertEquals('stdClass', get_class($service),
			'Array Definition: service [std] is not a "strClass" instance'
		);

		$container->remove('std');

		$this->assertFalse($container->has('std'),
			'service "std" not removed!'
		);
		...
		$this->assertEquals('a1', $service->arg1,
			'Argument [0] for constructor, not expected value'
		);
		$this->assertEquals('a2', $service->arg2,
			'Argument [1] for constructor, not expected value'
		);
		...
		$this->assertEquals('arg_value1', $service->arg1,
			'Argument [0] for constructor, not expected value'
		);
		$this->assertEquals('arg_value2', $service->arg2,
			'Argument [1] for constructor, not expected value'
		);
